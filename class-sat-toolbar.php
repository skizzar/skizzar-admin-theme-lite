...
class Skizzar_Admin_Theme_Toolbar {
	...
	static function action_add_toolbar_nodes_sooner( $wp_toolbar ) {
		...
		if ( ! Skizzar_Admin_Theme_Options::get_saved_option( 'enable-admin-theme' ) ) {
			return;
		}

		// Add Logo to admin bar
            if (Skizzar_Admin_Theme_Options::get_saved_option( 'logo-image' ) && ! Skizzar_Admin_Theme_Options::get_saved_option( 'hide-menu-logo' ) && Skizzar_Admin_Theme_Options::get_saved_option( 'enable-site-toolbar-theme' )) {
                    $html = '<img class="sat-menu-logo" src="' . Skizzar_Admin_Theme_Options::get_saved_option( 'logo-image' ) . '" title="' . get_bloginfo( 'name' ) . '" width="60px">';
